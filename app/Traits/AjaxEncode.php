...
trait AjaxEncode
{
    ...
    protected function ajaxBlogHtml(array $data)
    {
        try {
            $result = "";
            if (empty($data['onlyBlogList'])) {
                throw new \Exception("blog 列表数据为空！！！");
            }
            foreach ($data['onlyBlogList'] as $item) {
                $tagHtml = '';
                if(!empty($item['tags'])) {
                    $tagHtml = '<div class="dropdown-menu">';
                    foreach ($item['tags'] as $tag) {
                        $tagHtml .= '<a class="dropdown-item" href=" ' . $tag['tag_slug'] . '"> '. $tag['tag_name'] . '</a>';
                    }
                    $tagHtml .= '</div>';
                }
                $blogUrl = route('home.blog_details', ['id' => $item['id']]);
                $result .= <<<ERT
<div class="col-md-4">
                                <div class="card mb-4 shadow-sm">
                                    ...
                                    <div class="card-body jay-blog-card-body">
                                        <h5 class="card-text">
                                            <a style="max-width: 347px;" class="text-dark a-remove-line d-inline-block text-truncate" href="{$blogUrl}">{$item['title']}</a>
                                        </h5>
...
ERT;
    ...
    protected function ajaxHomeHtml(array $data)
    {
        try {
            $result = "";
            if (empty($data['onlyBlogList'])) {
            ...
            foreach ($data['onlyBlogList'] as $item) {
                $route = route('home.blog_details', ['id' => $item['id']]);
                $result .= <<<ERT
<div class="blog-post">
